...
function extractNameFromTemplate(value) {
  return value.replace(/Hello,\s/g, '').replace(/!/g, '');
}
...
function getRectangleString(width, height) {
  let top = "┌".concat("─".repeat(width - 2), "┐", "\n");
  let bottom = "└".concat("─".repeat(width - 2), "┘", "\n");
  let lr = "│".concat(" ".repeat(width - 2), "│", "\n");

  return top.concat(lr.repeat(height - 2), bottom);
}
